...
import { DatePicker } from '../../src/adapters/DatePicker';
...
export const EventDatePicker: Story = {
  render: () => {
    // Fixed initial date for deterministic testing
    const [eventDate, setEventDate] = useState<Date | null>(new Date('2024-07-20'));
    
    // Fixed reference date for deterministic testing
    const referenceDate = new Date('2024-06-15');
    const daysUntilEvent = eventDate 
      ? Math.ceil((eventDate.getTime() - referenceDate.getTime()) / (1000 * 60 * 60 * 24))
      : null;
...
export const MultipleDatePickers: Story = {
  render: () => {
    ...
    const [startDate, setStartDate] = useState<Date | null>(new Date('2024-06-15'));
